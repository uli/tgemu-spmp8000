...
void print_options(void)
{
    ...
    printf(" -fps <on|off>       \t Show FPS meter\n");
    ...
    printf(" -wave <file.wav>    \t Log sound output to 'file.wav'\n");
    printf(" -joy <s>            \t Select joystick driver (auto)\n");
    printf(" -split <yes|no>     \t Split ROM image at 2 megabit boundary\n");
    printf(" -flip <yes|no>      \t Bit-flip image\n");
    printf(" -usa <yes|no>       \t Enable TurboGrafx-16 system detection\n");
}
...
void parse_args(int argc, char **argv)
{
    ...
    for(i = 0; i < argc; i += 1)
    {
        ...
        if(stricmp("-auto", argv[i]) == 0)
        {
            option.autores = 1;

            if(option.autores)
            {
                option.autores_w = atoi(argv[i+1]);
                option.autores_h = atoi(argv[i+2]);
            }
        }
        ...
        if(stricmp("-fps", argv[i]) == 0)
        {
            option.fps = check_bool(argv[i+1]);
        }
        ...
        if(stricmp("-wave", argv[i]) == 0)
        {
            option.wave = 1;

            /* Ensure filename isn't actually another option or the last in the line */
            if(((i + 1) < argc) && (argv[i+1][0] != '-')) strcpy(option.wavpath, argv[i+1]);
        }
        ...
        if(stricmp("-split", argv[i]) == 0)
        {
            option.split = check_bool(argv[i+1]);
        }

        if(stricmp("-flip", argv[i]) == 0)
        {
            option.flip = check_bool(argv[i+1]);
        }

        if(stricmp("-usa", argv[i]) == 0)
        {
            option.usa = check_bool(argv[i+1]);
        }
    }
